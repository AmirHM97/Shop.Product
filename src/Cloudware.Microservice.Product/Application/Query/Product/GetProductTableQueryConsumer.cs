...
{
    public class GetProductTableQueryConsumer : IConsumer<GetProductTableQuery>, IMediatorConsumerType
    {
        private readonly IProductReadDbContext _productReadDbContext;
        private readonly ITableDataManager _dataManager;

        public GetProductTableQueryConsumer(IProductReadDbContext productReadDbContext, ITableDataManager dataManager)
        {
            _productReadDbContext = productReadDbContext;
            _dataManager = dataManager;
        }

        public async Task Consume(ConsumeContext<GetProductTableQuery> context)
        {
            //     var skip=(context.Message.SearchDto.PageNumber-1)*context.Message.SearchDto.PageSize;
            //     var productQuery = _productReadDbContext.ProductItemsDataCollection.Find(f => f.TenantId == context.Message.TenantId && f.Name.Contains(context.Message.SearchDto.Search)).Skip(skip).Limit(context.Message.SearchDto.PageSize);

            // if (context.Message.IsAdvance)
            // {

            //     mapField =
            //    new List<MapField>
            //    {
            //     new MapField{Name="ImageUrl",Title="#",Order=1,Type=TableValueType.Image},
            //     new MapField{Name="Code",Title="شناسه",Order=2,Type=TableValueType.Text},
            //     new MapField{Name="Name",Title="عنوان محصول",Order=3,Type=TableValueType.Text},
            ...
            // else
            // {
            var mapField = new List<MapField>
                             {
                new MapField{Name="ImageUrl",Title="#",Order=1,Type=TableValueType.Image},
                new MapField{Name="Code",Title="شناسه",Order=2,Type=TableValueType.Text},
                new MapField{Name="Name",Title="عنوان محصول",Order=3,Type=TableValueType.Text,Size=30},
                new MapField{Name="TotalCount",Title="موجودی",Order=5,Type=TableValueType.Text},
                new MapField{Name="MaxPrice",Title="قیمت محصول",Order=7,Type=TableValueType.Price},
                new MapField{Name="IsAvailable",Title="وضعیت",Order=8,Type=TableValueType.Status,StatusColors=new List<(string Name, string Title, string Color)>(){("true","فعال", "rgb(52,195,143)"), ("false", "غیر فعال", "rgb(244,106,106)") },Size=10},
                             };
            // }
            ...
            await context.RespondAsync(new GetProductTableQueryResponse(res));
        }
    }
    public class GetProductTableQueryResponse
    {
        public TableData TableData { get; set; }

        public GetProductTableQueryResponse(TableData tableData)
        {
            TableData = tableData;
        }
    }
